...
class UltraHighFrequencyBot:
    def __init__(self):
        ...
        self.equity_curve = [100.0]  # Fixed to match initial balance
        self.peak_equity = 100.0     # Fixed to match initial balance
        ...
        self.tp_percent = 0.014      # Smaller TP for faster hits (0.5%)
        self.sl_percent = 0.007     # Smaller SL (0.4%)
        self.position_size = 0.5    # 20% position size
    ...
    def show_real_time_dashboard(self, current_price, signal_data, market_conditions):
        ...
        print(f"📉 Drawdown: {current_drawdown:.2f}% | Max: {self.max_drawdown:.2f}%")
    ...
    def execute_trade(self, signal, current_price, market_conditions):
        ...
        # Position sizing with 20% of balance
    ...
    def run_bot(self):
        ...
        try:
            while True:
                ...
                # Fetch data quickly - CHANGED TO 200 CANDLES
                data = self.data_manager.fetch_historical_data(limit=200)  # Increased from 100 to 200
                ...
                # Show compact dashboard with drawdown
            ...
            entry_fees = sum(trade.get('entry_fee', 0) for trade in self.trade_log if 'entry_fee' in trade)
